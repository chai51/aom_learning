...
export class MotionFieldEstimation {
  ...
  motion_field_estimation() {
    ...
    let refStamp = MFMV_STACK_SIZE - 2;
    ...
    for (let y8 = 0; y8 < h8; y8++) {
      for (let x8 = 0; x8 < w8; x8++) {
        ...
        if (srcRef > REF_FRAME.INTRA_FRAME) {
          ...
          let posValid = Number(Math.abs(refToCur) <= MAX_FRAME_DISTANCE && Math.abs(refOffset) <= MAX_FRAME_DISTANCE && refOffset > 0);
          if (posValid) {
  ...
  get_mv_projection(mv: number[], numerator: number, denominator: number) {
    let clippedDenominator = Math.min(MAX_FRAME_DISTANCE, denominator);
    let clippedNumerator = Clip3(-MAX_FRAME_DISTANCE, MAX_FRAME_DISTANCE, numerator);
    let projMv: number[] = [];
    for (let i = 0; i < 2; i++) {
  ...
  get_block_position(x8: number, y8: number, dstSign: number, projMv: number[]) {
    ...
    let data = { posValid: 1 };
    this.PosY8 = this.project(y8, projMv[0], dstSign, cis.MiRows >> 1, MAX_OFFSET_HEIGHT, data);
    this.PosX8 = this.project(x8, projMv[1], dstSign, cis.MiCols >> 1, MAX_OFFSET_WIDTH, data);
  ...
  reference_frame_update() {
    ...
    for (let i = 0; i < NUM_REF_FRAMES; i++) {
      if (((fh.refresh_frame_flags >> i) & 1) == 1) {
        ...
        for (let j = 0; j < REFS_PER_FRAME; j++) {
